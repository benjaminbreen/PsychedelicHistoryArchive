...
export type ReaderTabId = "overview" | "transcript" | "historicalContext" | "translation" | "original" | "sourcePdf" | "details";
...
export function buildReaderTabs(source: ArchiveSource, transcript: string[]): ReaderTab[] {
  ...
  const hasTranscriptSections = source.transcriptSections?.some((section) => section.kind === "transcript");
  const hasTranscript = transcript.some((paragraph) => paragraph.trim()) || Boolean(hasTranscriptSections);
  const hasHistoricalContext = source.transcriptSections?.some((section) => section.kind === "historical_context");

  if (hasTranslation) {
    tabs.push({ id: "translation", label: "Translation" });
    if (hasTranscript) {
      tabs.push({ id: "transcript", label: originalTranscriptTabLabel(source) });
    }
  ...
  if (hasHistoricalContext) {
    tabs.push({ id: "historicalContext", label: "Historical context" });
  }
...
export function buildReaderToc(source: ArchiveSource, activeTab: ReaderTabId, transcript: string[]): ReaderTocItem[] {
  ...
  if (activeTab === "historicalContext") {
    const contextSections = source.transcriptSections?.filter((section) => section.kind === "historical_context") ?? [];
    const toc = contextSections.flatMap((section) => {
      if (section.bodyFormat === "markdown" && section.body) return extractMarkdownToc(section.body);
      return section.heading?.trim() ? [{ id: readerSectionId(section.heading), label: section.heading }] : [];
    });
...
function originalTranscriptTabLabel(source: ArchiveSource) {
  const language = (source.contentLanguage || source.language || "").trim();
  if (language && !/^english$/i.test(language)) return `Original ${language}`;
  return "Transcript";
}
