...
export function SourceCard({ match, source, featured = false }: SourceCardProps) {
  ...
  return (
    <article className="group grid h-full content-start rounded-md border border-transparent p-2 transition duration-200 hover:border-archive-line/80 hover:bg-archive-surface hover:shadow-[0_12px_28px_rgb(var(--archive-shadow)/0.08)]">
      <div className={featured ? "grid gap-4 sm:grid-cols-[8rem_1fr]" : "grid gap-5"}>
        <Link href={href} aria-label={`View source: ${source.title}`}>
          <SourceImage
        ...
        <div className="max-w-[21rem]">
          ...
          {match?.snippet && (
            <p className="mt-3 line-clamp-3 border-l-2 border-archive-violet/45 pl-3 text-[0.82rem] leading-5 text-archive-muted">
              <span className="font-semibold text-archive-violet">{match.label}:</span>{" "}
              {match.snippet}
            </p>
          )}
          <Link href={href} className="mt-4 inline-flex text-[1.02rem] leading-6 text-archive-ink underline decoration-archive-ink/70 underline-offset-4 transition hover:text-archive-violet hover:decoration-archive-violet focus-ring rounded-sm">
            Read more
          </Link>
        </div>
      </div>
